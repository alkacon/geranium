...
public class SimpleDecoratedPanel extends Composite implements I_Truncable {
     ...
     */
    protected interface I_CmsSimpleDecoratedPanelUiBinder extends UiBinder<Widget, SimpleDecoratedPanel> {
     ...
     */
    public SimpleDecoratedPanel(int decorationWidth, Widget mainWidget, List<Widget> decoration) {
    ...
    public void truncate(String textMetricsPrefix, int widgetWidth) {
        ...
        for (Widget widget : m_primary) {
            if (widget instanceof I_Truncable) {
                ((I_Truncable)widget).truncate(textMetricsPrefix, width);
            }
        }
    }
